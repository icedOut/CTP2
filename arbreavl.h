...
int  ArbreAVL<T>::hauteur() const {
    ...
    while (noeud!=NULL) {
        h++;
        noeud = (noeud->equilibre==1) ? noeud->gauche : noeud->droite;
    }
...
template <class T>
const T& ArbreAVL<T>::max() const {
    return (!vide()) ? max(racine) : NULL;
}
template <class T>
...
typename ArbreAVL<T>::Iterateur ArbreAVL<T>::rechercherEgalOuSuivant(const T& e) const {
    Iterateur iter(*this);
    Noeud* noeud = racine, *premier=NULL;
...
typename ArbreAVL<T>::Iterateur ArbreAVL<T>::rechercherEgalOuPrecedent(const T& e) const {
    Iterateur iter(*this);
    Noeud* noeud = racine, *dernier=NULL;
...
typename ArbreAVL<T>::Iterateur& ArbreAVL<T>::Iterateur::operator++() {
    ...
    courant = (!chemin.vide()) ? chemin.depiler() : NULL;
